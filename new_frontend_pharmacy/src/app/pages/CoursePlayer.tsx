...
import { completeLesson, getCourseById, getLessonsByCourse, getMyEnrollments } from '../lib/api';
...
export function CoursePlayer() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const [courseData, lessonsData, enrollments] = await Promise.all([
  ...
  const lessonProgressPercent = lessons.length > 0 ? (completedCount / lessons.length) * 100 : progressPercent;
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      await completeLesson(selectedLessonId, token);
      setCompletedLessonIds((prev) => (prev.includes(selectedLessonId) ? prev : [...prev, selectedLessonId]));
    } catch {
      // keep player responsive
    }
  };
